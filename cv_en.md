...
- The coach is not a bolt-on chatbot: every tool is **scoped per athlete** and carries its own domain **guardrails** (patch semantics on corrections, no writes to the global catalogue, explicit confirmation before significant changes, clarification requests when the ask is ambiguous)
...
- Integration with **Anthropic AI models** also for smart plan import, conversational memory with compaction and proactive reports, with **end-to-end cost control**: per-subscription spend caps, a single chokepoint that refuses calls without a declared payer, metering, model triage and budget gates
